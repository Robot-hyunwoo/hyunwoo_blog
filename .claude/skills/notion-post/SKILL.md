...
3. frontmatter의 `category: TBD`를 확정값으로 바꾸고 `src/content/posts/<슬러그>.md`로 저장한다.
   ...
   - **published 시각 규칙**: `src/content/posts/`에 오늘 날짜로 발행된 글이 이미 있으면(같은 날 2편째),
     이번 글의 published를 `YYYY-MM-DDTHH:MM:SS+09:00` 형식(현재 시각 + KST)으로 바꾸고,
     기존 오늘 글들도 시각이 없으면 순서가 유지되도록 시각을 부여한다. 타임존 `+09:00`은 필수
     (아카이브가 UTC로 해석해 날짜가 밀리는 것을 방지). 하루 1편이면 날짜만으로 충분하다.
